 ...
 *
 * One unified 5-step wizard covering both import paths this plugin
 * supports: Flamingo (this site's own live Flamingo data, or a CSV exported
 * from Flamingo's own Export button — recognized shape: arbitrary CF7-style
 * field-name columns ending in a `Date` column; see
 * {@see \InboxAI\Migration\FlamingoImporter}/{@see \InboxAI\Migration\FlamingoCsvImporter}),
 * and Inbox AI CSV (a CSV shaped for this plugin's own columns directly —
 * `source_category`, `category`, `priority`, `confidence`, `workflow_status`,
 * … — useful for loading test/demo data; see
 * {@see \InboxAI\Migration\InboxCsvImporter}'s docblock for the exact
 * recognized header).
 *
 * Step 1 (Type) picks which of those two the rest of the wizard is for; step
 * 2 (Source) then shows either the Flamingo live/CSV sub-choice or the
 * Inbox AI CSV upload panel; steps 3–5 (Options/Import/Complete) are shared
 * by both paths. `src/admin/componets/settings/flamingoImportTab.js` wires
 * all of this up in one file — there is deliberately no second Settings tab
 * or second JS file for the Inbox AI CSV path; it was merged in here rather
 * than duplicating the wizard shell.
 *
...
<section class="inboxai-screen<?php echo 'flamingo' === $active_tab ? ' inboxai-is-active' : ''; ?>" id="screen-flamingo">
	<div class="inboxai-page-header">
		<div>
			<h1><?php esc_html_e( 'Import & Migration', 'inbox-ai' ); ?></h1>
			<p><?php esc_html_e( 'Import data from Flamingo, or a CSV shaped for this plugin\'s own columns.', 'inbox-ai' ); ?></p>
		</div>
	</div>
	<div class="inboxai-settings__shell">
		...
		<div>
			<div class="inboxai-wizard__steps">
				<div class="inboxai-wizard__step inboxai-is-active" data-wizard-step="1"><div class="inboxai-wizard__circle">1</div><span><?php esc_html_e( 'Type', 'inbox-ai' ); ?></span></div>
				<div class="inboxai-wizard__line"></div>
				<div class="inboxai-wizard__step" data-wizard-step="2"><div class="inboxai-wizard__circle">2</div><span><?php esc_html_e( 'Source', 'inbox-ai' ); ?></span></div>
				<div class="inboxai-wizard__line"></div>
				<div class="inboxai-wizard__step" data-wizard-step="3"><div class="inboxai-wizard__circle">3</div><span><?php esc_html_e( 'Options', 'inbox-ai' ); ?></span></div>
				<div class="inboxai-wizard__line"></div>
				<div class="inboxai-wizard__step" data-wizard-step="4"><div class="inboxai-wizard__circle">4</div><span><?php esc_html_e( 'Import', 'inbox-ai' ); ?></span></div>
				<div class="inboxai-wizard__line"></div>
				<div class="inboxai-wizard__step" data-wizard-step="5"><div class="inboxai-wizard__circle">5</div><span><?php esc_html_e( 'Complete', 'inbox-ai' ); ?></span></div>
			</div>

			<!-- STEP 1: TYPE -->
			<div class="inboxai-card" id="flamingo-panel-1">
				<div class="inboxai-card__header"><h2><?php esc_html_e( 'What do you want to import?', 'inbox-ai' ); ?></h2><span class="inboxai-card__muted"><?php esc_html_e( 'The rest of this wizard adapts to whichever you choose', 'inbox-ai' ); ?></span></div>
				<div class="inboxai-card__body">
					<label class="inboxai-switch-row" style="cursor:pointer;">
						<div>
							<div class="inboxai-switch-row__text"><?php esc_html_e( 'Flamingo', 'inbox-ai' ); ?></div>
							<div class="inboxai-switch-row__sub"><?php esc_html_e( 'Migrate data from the Flamingo plugin — this site\'s own live data, or a CSV exported from Flamingo\'s own Export button', 'inbox-ai' ); ?></div>
						</div>
						<input type="radio" name="flamingo-type" id="flamingo-type-flamingo" value="flamingo" checked>
					</label>
					<label class="inboxai-switch-row" style="border-bottom:none;cursor:pointer;">
						<div>
							<div class="inboxai-switch-row__text"><?php esc_html_e( 'Inbox AI CSV', 'inbox-ai' ); ?></div>
							<div class="inboxai-switch-row__sub"><?php esc_html_e( 'Upload a CSV shaped for this plugin\'s own columns directly — useful for loading test/demo data', 'inbox-ai' ); ?></div>
						</div>
						<input type="radio" name="flamingo-type" id="flamingo-type-native" value="native">
					</label>
				</div>
				<div class="inboxai-modal__footer" style="justify-content:flex-end;background:transparent;">
					<button class="inboxai-btn--primary" id="flamingo-next-1"><?php esc_html_e( 'Next', 'inbox-ai' ); ?></button>
				</div>
			</div>

			<!-- STEP 2: SOURCE -->
			<div class="inboxai-card" id="flamingo-panel-2" style="display:none;">
				<div class="inboxai-card__header"><h2 id="flamingo-source-title"><?php esc_html_e( 'Choose a Source', 'inbox-ai' ); ?></h2><span class="inboxai-card__muted" id="flamingo-source-sub"><?php esc_html_e( 'Read this site\'s live Flamingo data, or upload a CSV exported from Flamingo', 'inbox-ai' ); ?></span></div>
				<div class="inboxai-card__body">

					<!-- Flamingo sub-choice: live data vs Flamingo's own CSV export -->
					<div id="flamingo-source-flamingo-wrap">
					...
					<!-- Inbox AI CSV upload (only shown when Step 1 chose "Inbox AI CSV") -->
					<div id="flamingo-source-native-wrap" style="display:none;text-align:center;padding:28px 0 8px;">
						<p style="margin:0 0 14px;font-size:12.5px;color:var(--text-secondary);"><?php esc_html_e( 'Must include at least "sender_email" and "message" columns — see Notes below', 'inbox-ai' ); ?></p>
						<div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;justify-content:center;">
							<label class="inboxai-btn--secondary" for="flamingo-native-file-input" style="cursor:pointer;margin:0;"><?php esc_html_e( 'Choose File', 'inbox-ai' ); ?></label>
							<input type="file" id="flamingo-native-file-input" accept=".csv" style="display:none;">
							<span id="flamingo-native-file-name" style="font-size:12.5px;color:var(--text-tertiary);"><?php esc_html_e( 'No file chosen', 'inbox-ai' ); ?></span>
						</div>
						<div id="flamingo-native-detected-info" style="display:none;align-items:center;justify-content:center;gap:8px;background:var(--conf-good-soft);border:1px solid #BEE7CD;border-radius:8px;padding:10px 14px;font-size:12.5px;color:var(--conf-good);font-weight:600;margin:14px auto 0;max-width:420px;">
							<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" style="width:13px;height:13px;flex-shrink:0;"><path d="M20 6L9 17l-5-5"/></svg>
							<span></span>
						</div>
					</div>
				...
				<div class="inboxai-modal__footer" style="justify-content:space-between;background:transparent;">
					...
					<button class="inboxai-btn--primary" id="flamingo-next-2" disabled><?php esc_html_e( 'Next', 'inbox-ai' ); ?></button>
				</div>
			</div>
			...
			<div class="inboxai-card" id="flamingo-panel-3" style="display:none;">
				...
				<div class="inboxai-card__body">
					...
					<div class="inboxai-switch-row" style="border-bottom:none;"><div><div class="inboxai-switch-row__text" id="flamingo-toggle-ai-text"><?php esc_html_e( 'Run AI analysis on imported messages', 'inbox-ai' ); ?></div><div class="inboxai-switch-row__sub"><?php esc_html_e( 'Optional — can also be run later in smaller batches to manage cost', 'inbox-ai' ); ?></div></div><div class="inboxai-switch" id="flamingo-toggle-ai"></div></div>
				</div>
				<div class="inboxai-modal__footer" style="justify-content:space-between;background:transparent;">
					<button class="inboxai-btn--secondary" id="flamingo-back-3"><?php esc_html_e( 'Back', 'inbox-ai' ); ?></button>
					<button class="inboxai-btn--primary" id="flamingo-next-3"><?php esc_html_e( 'Next', 'inbox-ai' ); ?></button>
				</div>
			</div>
			...
			<div class="inboxai-card" id="flamingo-panel-4" style="display:none;">
				...
				<div class="inboxai-card__body">
					...
					<div class="inboxai-info-row"><span><?php esc_html_e( 'Rows to import', 'inbox-ai' ); ?></span><b style="font-family:var(--mono);" id="flamingo-summary-messages">0</b></div>
				...
				<div class="inboxai-modal__footer" style="justify-content:space-between;background:transparent;">
					<button class="inboxai-btn--secondary" id="flamingo-back-4"><?php esc_html_e( 'Back', 'inbox-ai' ); ?></button>
					<button class="inboxai-btn--primary" id="flamingo-start-import-btn"><?php esc_html_e( 'Start Import', 'inbox-ai' ); ?></button>
				</div>
			</div>

			<!-- STEP 5: COMPLETE -->
			<div class="inboxai-card" id="flamingo-panel-5" style="display:none;">
			...
			<div class="inboxai-card" style="margin-top:16px;">
				...
				<div class="inboxai-card__body" style="font-size:12.5px;color:var(--text-secondary);line-height:1.7;">
					<p style="margin:0;"><?php esc_html_e( 'Flamingo: original Flamingo entries are left untouched — this creates copies inside AI Inbox. You can re-run this import later for any new Flamingo messages received after today.', 'inbox-ai' ); ?></p>
					<p style="margin:10px 0 0;"><?php esc_html_e( 'Inbox AI CSV recognized columns (any order, case-insensitive; only "sender_email" and "message" are required):', 'inbox-ai' ); ?></p>
					<code style="display:block;background:var(--surface-2);border:1px solid var(--border);border-radius:6px;padding:10px 12px;font-size:12px;line-height:1.8;white-space:pre-wrap;word-break:break-word;margin-top:6px;">sender_name, sender_email, phone, company, form_title, source_category, subject, message, category, priority, confidence, workflow_status, created_at</code>
					<p style="margin:10px 0 0;"><?php esc_html_e( '"source_category" is the fixed, form-defined category (never changed by AI regenerate). "category" is the AI\'s own classification — provide it directly, or leave it blank and enable "Run AI analysis" to have it generated for real. Re-uploading the same file is safe either way — rows already imported before are skipped, not duplicated.', 'inbox-ai' ); ?></p>
				</div>
			</div>
		</div>
