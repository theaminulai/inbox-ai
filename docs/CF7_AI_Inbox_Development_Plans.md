...
Settings has to exist first because nothing can call an AI provider or know which forms to watch without it. AI Inbox List has to exist before Overview, Contacts, or Analytics because all three are just different views over the same `inboxai_messages` / `inboxai_activities` / `inboxai_usage` rows that only AI Inbox List's capture pipeline produces. Building Overview or Analytics first would mean staring at empty-state screens with nothing to develop against.

---

## 1. Overview page (`inboxai-overview`, was "Dashboard" in the mockup)
...
- An AJAX action (e.g. `inboxai_get_overview`, nonce-protected, `current_user_can( 'inboxai_view_messages' )`) that returns this payload as JSON, replacing the current mock `messages` array in `common.js`.
...
## 2. AI Inbox List page (`inboxai-inbox`)
...
- `includes/CF7/SubmissionMapper.php` — maps CF7's posted data + form fields to the `inboxai_messages` columns, excluding passwords/sensitive fields per R&D §5.1, and computing `submission_hash` to prevent duplicate rows on retried submits.
...
### Steps
1. `SubmissionHandler` + `SubmissionMapper`, wired only for forms enabled in Settings — verify a real CF7 submission lands a row in `inboxai_messages`.
...
## 3. Contacts List page (`inboxai-contacts`)
...
R&D §6 marks a dedicated `inboxai_contacts` table as "optional for version 1" and recommends deriving contacts from `inboxai_messages` at query time initially. This plan follows that recommendation: no new table yet. One open question to resolve before writing the delete endpoint — since there's no real contacts table, "Delete contact" (currently just a client-side `Set` of hidden emails in the mockup) needs a real meaning. The two options are (a) archive every message from that sender rather than truly deleting data, or (b) skip a destructive action entirely for v1 and only ship "View messages." Recommend (a), gated behind `inboxai_delete_messages`, since it reuses Plan 2's existing archive logic instead of inventing new deletion semantics.
...
### Frontend work
`contacts.js` already renders real `<a href="inbox.html?search=...">`-style links (now pointing at the real `inboxai-inbox` admin URL instead of a static file) — only the data source changes from the client-side `contactsFromMessages()` derivation to a server-side aggregate call.
...
## 4. Analytics page (`inboxai-analytics`)
...
## 5. Settings page (`inboxai-settings`)
...
- AI Provider tab: `ProviderFactory`/`AIService` (R&D §8) power "Test Connection" and a live "Load Models" call per provider — both AJAX actions gated by `inboxai_manage_settings`, since they need the (unsaved) API key from the form before it's persisted.
