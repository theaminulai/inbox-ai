# End-to-End Plan: Campaigns Page (`inboxai-campaigns`, `html/campaign.html`)
...
CREATE TABLE {prefix}inboxai_campaign_recipients (
  id            BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
...
- Two new capabilities in `includes/Security/Capabilities.php`, mirroring the existing `VIEW_MESSAGES`/`SEND_REPLIES` split (viewing is safe; bulk-sending is the one genuinely risky action on this page and deserves a narrower grant than viewing does):
  ```php
  public const VIEW_CAMPAIGNS = 'inboxai_view_campaigns';
  public const SEND_CAMPAIGNS = 'inboxai_send_campaigns';
  ```
  Both added to `Capabilities::all()` and granted to Administrator on activation, same as every existing capability.
- `includes/Database/CampaignRepository.php` — `get_campaigns( $page, $per_page ): array{items, total}`, `find( int $id )`, `insert( array $data ): int`, `update_status( int $id, string $status )`, `duplicate( int $id ): int`, `delete( int $id )`, `record_recipients( int $id, array $emails ): void` (bulk-inserts `inboxai_campaign_recipients` rows with fresh `send_token`s), `record_open( string $token ): bool`, `open_rate( int $id ): ?float`.
...
- `includes/Services/CampaignSendService.php` — resolves the audience via `audience_contacts()`, chunks it by the campaign's `send_rate`, and queues sends using the **same** Action-Scheduler/WP-Cron queue infrastructure Plan 2 built for AI analysis jobs (section 3.2 there) rather than standing up a second queue mechanism. Per recipient: generates a `send_token`, embeds a 1×1 tracking pixel pointed at `inboxai_track_open&t=<token>` in the sent body, calls the active `EmailProviderInterface::send()`, and records the `sent_at` timestamp. Re-resolves the audience fresh at the start of each day's batch (not once at wizard-submit time) — see section 6 for why. Updates `inboxai_campaigns.status`/`sent_at` as batches complete (`sending` while multi-day batching is in progress, `sent` once the last batch finishes).
- `includes/Admin/Pages/CampaignsPage.php` — same thin shape as the other five page classes: capability check (`inboxai_view_campaigns` in `render()`, re-checked per-action at `inboxai_send_campaigns` for writes), assembles a view model (current campaigns page + the four `distinct_values()` lists the wizard's segment dropdown needs), calls `Support\Template::render( 'campaigns/campaigns', $view_model )`, which in turn renders `campaigns/list.php` and `campaigns/wizard.php` — following the `inbox/`/`settings/` subfolder convention this session's Templates reorg already established for the other two multi-screen pages. **No enqueue call**: `Menu.php` enqueues the shared bundle for every registered page; this class's constructor hooks `inboxai_localize_data`, checks `$slug === 'inboxai-campaigns'`, and adds its nonce(s) — same mechanism as every other page (`docs/plans/05-settings-plan.md` §10).
- `includes/Admin/Ajax/CampaignsAjaxController.php` — **not one shared `AjaxController` class, as this line originally said.** As of the Contacts List build (Plan 3), the single shared controller was split into one class per admin page under `includes/Admin/Ajax/`, each extending a shared `BaseAjaxController` for just the nonce + `current_user_can()` check; this page should add its own `CampaignsAjaxController` the same way. New actions:

| Action | Capability | Notes |
|---|---|---|
| `inboxai_list_campaigns` | `VIEW_CAMPAIGNS` | list + pagination |
| `inboxai_get_audience_count` | `VIEW_CAMPAIGNS` | wizard Step 1's live count, replaces client-side `computeAudience().length` |
| `inboxai_create_campaign` | `SEND_CAMPAIGNS` | draft / schedule / send-now, per the wizard's final step |
| `inboxai_duplicate_campaign` | `SEND_CAMPAIGNS` | |
| `inboxai_delete_campaign` | `SEND_CAMPAIGNS` | |
| `inboxai_track_open` | *(none — see section 5)* | hit by an `<img>` tag inside a delivered email, can't carry a nonce |
...
- `api.js` — shared `fetch()`-to-`admin-ajax.php` wrapper (same as every other plan), reading from `window.inboxaiAdmin`.
- `list.js` — replaces the local `campaigns` mock array + `renderCampaigns()`'s client-side rendering with a `inboxai_list_campaigns` call.
- `audience.js` — `computeAudience()`/`distinctValues()`/`updateCampaignSegmentOptions()`/`updateCampaignAudienceCount()` replaced by a `inboxai_get_audience_count` call keyed on the selected type/value — the aggregate now happens in `MessageRepository::audience_contacts()`, not in the browser over the full mock `messages` array.
- `wizard.js` — `resetCampaignWizard()`/`goCampaignStep()`/`fillCampaignReview()`/`updateCampaignProviderDisplay()` kept essentially as-built (pure UI state machine, no data dependency beyond the audience count above); the Send button now calls `inboxai_create_campaign` and, for an immediate send, polls `inboxai_list_campaigns` (or a small dedicated status action) to animate the real progress bar instead of the mockup's `setInterval()` + `Math.random()` simulation.
...
- `inboxai_view_campaigns` gates the list/KPIs/audience-count preview (read-only); `inboxai_send_campaigns` gates every write action (create/duplicate/delete) — re-checked server-side per action, not just hidden client-side.
- The server re-validates, on `inboxai_create_campaign`, that a non-"all" audience has its consent switch confirmed — never trust the client-side check alone (the mockup's own `campaign-consent-warning` logic is a UX nicety, not a security boundary).
- `inboxai_track_open` is necessarily unauthenticated (an emailed `<img>` tag can't carry a WordPress nonce). Its `send_token` must be a long, unguessable random value 1:1 with a single `inboxai_campaign_recipients` row — never a sequential ID — and the handler only ever writes an `opened_at` timestamp; it never reads back or returns any campaign/recipient data, so a forged/guessed token can at most mark one open, never leak anything.
...
- Each of the four non-"all" audience types returns the same contact count a direct SQL query against seeded `inboxai_messages` rows would, matching `computeAudience()`'s existing client-side logic value-for-value.
- Full wizard round-trip (Audience → Compose → Sending → Review → Send) for both "send now" and "schedule for later" produces a real `inboxai_campaigns` row with the correct status, recipient rows, and tokens.
...
- Duplicate/Delete row actions work and are rejected server-side for a user with `inboxai_view_campaigns` but not `inboxai_send_campaigns`.
- `inboxai_track_open` with an invalid/guessed token silently no-ops (no error message, no data returned) rather than leaking whether a token exists.
...
2. `Migrator.php`: add `inboxai_campaigns` + `inboxai_campaign_recipients` tables.
...
8. `CampaignsPage` + the new `CampaignsAjaxController`'s actions from section 3's table.
...
10. Add `'inboxai-campaigns' => array( 'Campaigns', 'Campaigns', Capabilities::VIEW_CAMPAIGNS, CampaignsPage::class )` to `Menu::PAGES`.
...
- **Hard dependency on Plan 2** (AI Inbox List) — audience segmentation reads `inboxai_messages` exactly the way Plan 3 (Contacts) does; cannot be meaningfully built or tested without real captured submissions.
- **Hard dependency on a new Email Provider connection** — no existing plan covers the *real backend* for this. The static mockup side is now done: `html/settings.html` has a seventh subtab (`#screen-email-settings`, `data-subnav="email-settings"`) with the provider picker (SendGrid/Postmark/SES/Mailgun), API key field, Test Connection, and Sending Defaults card, wired in `html/assets/js/settings.js`, and the wizard's "Manage" link in `html/campaign.html` now points at `settings.html?tab=email-settings`. What's still missing is everything in section 3 — `EmailProviderInterface`/`ProviderFactory`, the real settings-repository fields, encryption, and the actual `inboxai_test_connection`-equivalent AJAX action. Recommend building the real backend as a small addendum to Plan 5 (Settings) rather than a new standalone plan, since it's shaped exactly like Plan 5 §3.2's AI-Provider-tab pattern.
- **Soft dependency on Plan 3's precedent** (no dedicated contacts table; audience is always derived live from `inboxai_messages`) — this plan follows that same call for consistency, per section 2.
