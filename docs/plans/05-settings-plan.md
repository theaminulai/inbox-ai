...
- `includes/Admin/Ajax/SettingsAjaxController.php` (as built: this page's own controller class, not the shared-by-every-page class this plan originally assumed — see section 10's note below) — handles the six save actions (one per tab, or one generic `inboxai_save_settings` action with a `tab` param) plus the read/test/import actions detailed in 3.2 and 3.7 below:
...
- Two of `SettingsAjaxController`'s actions (see the table in 3.1) are specific to this tab:
...
- `SettingsAjaxController` actions for this tab (see 3.1's table): `inboxai_flamingo_detect` (step 1 — count what's available), `inboxai_flamingo_import_batch` (step 3, called repeatedly with an offset until done).
...
- `api.js` — shared `fetch()`-to-`admin-ajax.php` wrapper (same as the other four plans), reading its bootstrap data from `window.inboxaiAdmin` (the one shared localized object every page gets — see section 10), not a page-specific global.
...
- **`Menu.php` owns all enqueuing, once.** `Menu::enqueue_assets()`, hooked to `admin_enqueue_scripts`, checks the current hook suffix against the hook suffixes `register_menu()` captured from `add_submenu_page()`'s return value, and bails immediately on every screen that isn't one of this plugin's own. On a match, it enqueues one shared script + style handle (`inboxai-admin` — as built; a redundant `-inbox-` segment in this handle was dropped project-wide in a later cleanup pass) pointing at `build/admin.js` / `build/admin.css`, using `build/admin.asset.php` (written by the wp-scripts/webpack build) for the real dependency array and a content-hash version string, and adds `wp_style_add_data( ..., 'rtl', 'replace' )` so `build/admin-rtl.css` swaps in automatically for RTL locales. **No page class enqueues anything itself.**
- **Page-specific localized data goes through one filter, not `wp_localize_script()` per page.** `Menu::enqueue_assets()` builds `array( 'ajaxUrl' => admin_url( 'admin-ajax.php' ) )`, runs it through `apply_filters( 'inboxai_localize_data', $data, $slug )`, and calls `wp_localize_script()` exactly once with the result, as `window.inboxaiAdmin` (both names as built — the same later cleanup pass dropped a redundant `_inbox_`/`Inbox` segment from the filter name and the JS global). Multiple `wp_localize_script()` calls on the same handle don't merge — only the last one survives — so a page that needs its own nonce or other bootstrap data hooks this one shared filter in its constructor and checks `$slug` before touching `$data`, exactly like `SettingsPage::localize_data()`:
  ```php
...
- **SCSS: `common/` for shared partials, one same-named folder per page for the rest.** `src/admin/scss/common/` holds partials every page uses (`_variables.scss`, `_base.scss`, `_buttons.scss`, `_card.scss`, `_fields.scss`, `_switch.scss`, `_modal.scss`, `_toast.scss`); each page gets its own lowercase folder next to it (`src/admin/scss/settings/`, `inbox/`, `contacts/`, and later `overview/`, `analytics/`) for partials only that page uses — as built, a page whose table/toolbar is visually identical to an earlier page's (Contacts List's grid-table vs. the AI Inbox List's) still gets its own folder for the one thing that actually differs (column widths), reusing the earlier page's generic rules rather than duplicating them. All of it is pulled into the one `src/admin/scss/index.scss` entry via `@use` — watch for Sass's default per-file namespace collisions when two pages each have their own same-named partial (e.g. two `_table.scss` files both default to namespace `table`); alias one with `as` (`@use 'contacts/table' as contacts-table;`).
- **`AjaxController` did *not* stay one shared class.** This was the plan for every page (section 3.1's table) through the Contacts List build, but that single file grew large enough (six Settings tabs + the Import wizard, eight AI Inbox List actions, plus Contacts' own actions) to become unwieldy. As built, it was split into `includes/Admin/Ajax/{SettingsAjaxController,InboxAjaxController,ContactsAjaxController}.php` — one class per admin page, each extending a small `BaseAjaxController` that holds just the shared nonce + `current_user_can()` check. Every reference to a bare `AjaxController` class elsewhere in these plans (including the code sample above) means that page's own controller under `includes/Admin/Ajax/`.
