# End-to-End Plan: Contacts List Page (`cf7ai-contacts`, `html/contacts.html`)
...
Standalone build plan for the third of five admin pages. This page has no data of its own — it's a grouped view over the same `cf7ai_messages` rows Plan 2 (AI Inbox List) captures, aggregated by sender email.
...
No new table. Every field comes from `{prefix}cf7ai_messages`, grouped by `sender_email`:
...
There is deliberately no dedicated `cf7ai_contacts` table (R&D §6 marks it optional for v1, and this plan follows that call). That means "delete" can't mean "remove a contact record" — there isn't one. Two real options:

- **(a) Archive-by-email** — set `workflow_status = 'archived'` on every message from that sender. Reuses Plan 2's existing archive logic and capability (`cf7ai_delete_messages`), no new destructive code path.
...
|---|---|---|
| `cf7ai_list_contacts` | `cf7ai_view_messages` | filters + pagination |
| `cf7ai_delete_contact` | `cf7ai_delete_messages` | calls `archive_by_email()` per section 3 |
| `cf7ai_export_contacts` | `cf7ai_export_messages` | mirrors `exportContactsCsv()` |
...
- `api.js` — shared `fetch()`-to-`admin-ajax.php` wrapper (same as Plans 1 and 2), reading from `window.cf7aiInboxAdmin` rather than a page-specific global.
- `list.js` — replaces `contactsFromMessages()` + `filteredContacts()` + `renderContacts()`'s client-side grouping with a `cf7ai_list_contacts` call keyed by `state.contactsFilters`/`state.contactsPage` — the grouping itself now happens in `get_contacts()` (section 4), not in the browser.
...
- "View messages" row action stays a plain `<a href="inbox.html?search=...">`-equivalent link, now pointed at the real AI Inbox List admin URL with the same `search` query arg (`Menu::url( 'cf7ai-inbox' ) . '&search=' . rawurlencode( $email )`), rendered server-side in `contacts-list.php`.
- `state.deletedContacts` (the client-side `Set`) goes away entirely — "Delete contact" calls `cf7ai_delete_contact` via `api.js`, then re-fetches the current page of contacts.
...
- `cf7ai_list_contacts` requires `cf7ai_view_messages`, nonce-checked, read-only.
- `cf7ai_delete_contact` requires `cf7ai_delete_messages` (a stronger capability than viewing), nonce-checked — this is the one write path on this page and deserves the same defense-in-depth capability re-check as every write action in Plan 2.
...
- Delete contact archives every message from that sender (verify in AI Inbox List, not just that the contact disappears here) and is blocked for a user without `cf7ai_delete_messages`.
...
3. `cf7ai_list_contacts` action.
4. Confirm the Delete decision (section 3) with the project owner if not already settled; implement `archive_by_email()` + `cf7ai_delete_contact` accordingly.
5. `cf7ai_export_contacts` action (or keep client-side, per section 5).
...
7. Add `'cf7ai-contacts' => array( 'Contacts List', 'Contacts List', Capabilities::VIEW_MESSAGES, ContactsListPage::class )` to `Menu::PAGES` — no iframe fallback exists to replace (removed entirely; see `docs/plans/05-settings-plan.md` §10). `Menu.php` automatically enqueues the shared bundle; `ContactsListPage`'s constructor just needs to hook `cf7ai_inbox_localize_data` for its nonce.
