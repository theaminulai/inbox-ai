# End-to-End Plan: Analytics Page (`inboxai-analytics`, `html/analytics.html`)
...
Everything here reads from `{prefix}inboxai_messages` and `{prefix}inboxai_activities`, both owned by Plan 2's schema — no new tables. Two additions to what Plan 2 logs are needed specifically for this page's KPIs:
...
- `includes/Admin/Pages/AnalyticsPage.php` — same thin shape as the other four page classes: capability check (`inboxai_view_analytics`, not `inboxai_view_messages` — see section 5), assemble the view-model, call `Support\Template::render( 'analytics', $view_model )` with markup in `includes/Templates/analytics.php`. **No enqueue call**: `Menu.php` enqueues the shared `build/admin.js`/`build/admin.css` bundle for every registered page; this class's constructor instead hooks the shared `inboxai_localize_data` filter, checking `$slug === 'inboxai-analytics'`, to add its own nonce — see `docs/plans/05-settings-plan.md` §10.
- `includes/Admin/Ajax/AnalyticsAjaxController.php` — **not the one shared `AjaxController` class this line originally described.** As of the Contacts List build (Plan 3), that single file was split into one controller class per admin page under `includes/Admin/Ajax/` (`SettingsAjaxController`, `InboxAjaxController`, `ContactsAjaxController` today), each extending a shared `BaseAjaxController` for just the nonce + `current_user_can()` check. This page should add its own `AnalyticsAjaxController` the same way. — `inboxai_get_analytics` (nonce `inboxai_analytics`, capability `inboxai_view_analytics` — note this is a *different* capability than the other pages use, already defined in `Capabilities.php` and already grantable to a role that shouldn't see raw message content but should see aggregate performance).

## 4. Frontend build plan (`src/admin/componets/analytics/` — net-new, not a rewire)

This page has no working JS today (`assets/js/analytics.js` is a placeholder), so there's no legacy logic to port — it's built directly as vanilla JS modules against `inboxai_get_analytics`. All of it compiles into the one shared `build/admin.js`/`build/admin.css` bundle, not a separate bundle per page (webpack.config.js disables code-splitting — see `docs/plans/05-settings-plan.md` §10):

- `api.js` — shared `fetch()`-to-`admin-ajax.php` wrapper (same as the other four plans), reading from `window.inboxaiAdmin` rather than a page-specific global.
...
- A user with `inboxai_view_analytics` but not `inboxai_view_messages` can load this page; a user with neither cannot.
...
4. `AnalyticsPage` view-model assembler + `inboxai_get_analytics` action.
...
6. Add `'inboxai-analytics' => array( 'Analytics', 'Analytics', Capabilities::VIEW_ANALYTICS, AnalyticsPage::class )` to `Menu::PAGES` — no iframe fallback exists to replace (removed entirely; see `docs/plans/05-settings-plan.md` §10). `Menu.php` automatically enqueues the shared bundle; `AnalyticsPage`'s constructor just needs to hook `inboxai_localize_data` for its nonce.
